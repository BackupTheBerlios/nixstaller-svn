pkg.enable = true
pkg.name = "bzflag"
pkg.version = "2.0.12"
pkg.release = "1"
pkg.summary ="Use your tank to shoot others and capture flags in a 3D multiplayer world."
pkg.description = [[
BZFlag is a free online multiplayer cross-platform open source 3D tank
battle game. The name originates from "Battle Zone capture the Flag".
]]
pkg.group = "Games"
pkg.license = "GPLv2"
pkg.maintainer = "Rick Helmus <rhelmus_AT_gmail>" -- You probably want to change this :-)
pkg.url = "bzflag.org"
pkg.bins = { "bin/bzfs" , "bin/bzflag", "bin/bzadmin" }
pkg.deps = { "Mesa", "OpenSSL", "SDL-1.2", "X11-core", "cURL", "core-libs", "core-libs-other", "libidn", "libstdc++", "zlib" }
