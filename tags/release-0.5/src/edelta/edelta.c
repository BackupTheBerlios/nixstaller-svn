...
#include <stdlib.h>
/*#include <zlib.h> Modified by Rick */
...
// 		gzsetparams(res,9,Z_DEFAULT_STRATEGY);
		...
		fwrite("edelta",6,1,res);
		fputc(EDELTA_PATCH_VERSION,res);
		...
		for(i=0; i<2; i++)
		{
			...
			fwrite(digest,sizeof(digest),1,res);
		}
	...
	else if(argc>=4 && !strcmp(*argv,"patch"))
	{
		...
		char header[6];
		fread(header,sizeof(header),1,patch);
		if( strncmp(header,"edelta",6) || fgetc(patch)!=EDELTA_PATCH_VERSION)
		{
			fprintf(stderr,"Unsupported patch version!\n");
			exit(-1);
		}

                fread(digest1,sizeof(digest1),1,patch);
                fread(digest2,sizeof(digest2),1,patch);
