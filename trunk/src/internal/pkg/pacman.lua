...
function present()
    return os.execute("pacman --version >/dev/null 2>&1") ~= 127
end
...
function genfilelist(src)
    ...
    while #dirlist > 0 do
        local dir = table.remove(dirlist) -- Pop
        local dirpath = string.format("%s/%s", src, (dir ~= "." and dir) or "")
        for f in io.dir(dirpath) do
            local path = string.format("%s/%s", dirpath, f)
            OLDG.install.print("path: " .. path)
            if os.isdir(path) then
                table.insert(dirlist, path)
            end
            
            check(flist:write(path .. "/" .. f .. "\n"))
        end
    end
